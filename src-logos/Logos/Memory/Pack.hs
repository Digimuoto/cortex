...
Description : Logos reasoning-library support for pack.
...
packSessionPassages tokenBudget maxCount rows =
  selectByTokenBudget
    ...
    maxCount
    (sortedRows rows)
    (.logosMemoryPassageTokenCount)
  where
    sortedRows =
      sortBy
        ( comparing (Down . (.logosMemorySourceUpdatedAt))
            <> comparing (Down . (.logosMemoryPassageOrder))
        )
  ...
  where
    greedyPack rs =
      selectByTokenBudget
        ...
        maxCount
        (sortedRows rs)
        (\row -> row.logosRankedPassage.logosMemoryPassageTokenCount)
    sortedRows =
      sortBy
        ( comparing (Down . logosRankedFinalScore)
            <> comparing (Down . (\row -> row.logosRankedPassage.logosMemorySourceUpdatedAt))
        )
...
-}
passageFeatures :: LogosMemoryRankedPassage -> Set Text
passageFeatures row =
  Set.fromList $
    fmap T.toLower row.logosRankedPassage.logosMemoryEntities
      <> maybeToList (T.toLower <$> row.logosRankedPassage.logosMemorySectionHeading)
      <> T.words (T.toLower row.logosRankedPassage.logosMemorySourceTitle)
...
selectByMMR lambda tokenBudget maxCount candidates =
  ...
  where
    ...
    maxScore = case candidates of
      (top : _) -> max 1e-9 (logosRankedFinalScore top)
      [] -> 1.0
                ...
                fst $
                  foldl'
                    ( \(bestSoFar, bestScore) (candidate, features) ->
                        let relevance = logosRankedFinalScore candidate / maxScore
                    ...
                    (first, -1e9)
                    (first : rest)
              tokens = fromIntegral (max 1 best.logosRankedPassage.logosMemoryPassageTokenCount)
