...
`validateNodeBoundaryNormalForm`, shared node-boundary egress wrappers, and
`planGraphRewriteWithAdmissionWitness` for runtime planner and budget equations. These checks are
executable drift detectors, not Lean proofs that the compiler produces every
