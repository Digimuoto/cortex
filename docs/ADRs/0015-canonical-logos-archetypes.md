---
title: "ADR 0015 — Canonical Logos Archetypes"
description:
  "Logos.Archetypes defines the canonical epistemological taxonomy for Logos reasoning modes: Logos,
  Sophia, Techne, Episteme, Kritikos, Themis, and Poiesis."
sidebar:
  label: "0015. Logos archetypes"
  order: 15
status: proposed
date: 2026-04-27
superseded_by: null
...
# ADR 0015 — Canonical Logos Archetypes

## Status

Proposed - establishes `Logos.Archetypes` as the canonical epistemological taxonomy and reserves
`Logos.Thought`, `Logos.Memory`, and `Logos.Patterns` for model-mediated cognition. Concrete
prompts, corpora, embedding indexes, tool surfaces, memory policies, evaluation suites, runtime
contracts, workflow templates, and role-contract refinements remain follow-up implementation work.

## Context

Logos needs a stable vocabulary for describing distinct modes of reasoning used by thoughts,
workflows, tools, and runtime structures built on Cortex. These modes should not be framed as
durable "agents" or "personas," because that language is too shallow and too identity-shaped for the
system being built. Logos models reasoning as structured work over evidence, judgment, construction,
critique, constraint, and expression.

We therefore define a canonical set of epistemological archetypes under `Logos.Archetypes`. `Logos`
is the downstream reasoning library built on Cortex. Its `Archetypes` children represent specialized
modes of cognition that can be composed, invoked, constrained, audited, and eventually made concrete
as thought frames, tool surfaces, prompt contracts, memory policy, or workflow policies.

`Logos.Thought` is reserved for one model-mediated cognitive evaluation bound to a graph node. A
thought has a frame, policy, prompt/context, memory surface, tool surface, and output contract. It
is not a durable persona.

`Logos.Memory` is reserved for cognitive memory: retrieval, ranking, packing, compaction,
...
context, the context-building API belongs in Logos memory while Pulse remains the substrate.

`Logos.Patterns` is reserved for reusable model-mediated reasoning programs built from those
...
The vertical split is runtime substrate below, structured reasoning library above. The canonical
reasoning-library root is `Logos`.
...
Adopt the following seven archetypes as the canonical Logos epistemological taxonomy under
`Logos.Archetypes`, and reserve the rest of the top-level Logos tree for thought execution,
...
| Root               | Owns                                                                                                                      |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------- |
| `Logos.Archetypes` | Semantic reasoning modes and their activation bundles.                                                                    |
| `Logos.Thought`    | One model-mediated node evaluation: frame, policy, prompt/context, tool loop, structured output, and thought events.      |
| `Logos.Memory`     | Cognitive memory, retrieval, packing, compaction, topological context, and memory tools.                                  |
| `Logos.Patterns`   | Reusable LLM-shaped reasoning programs assembled from archetypes, contracts, prompts, memory presets, and Wire templates. |

The default target for LLM-shaped reasoning modules is `Logos`, but exact homes are resolved per
module by ADR 0017's migration table. This ADR decides the conceptual taxonomy; it does not claim
that every current `Cortex.Agent`, `Cortex.Task`, `Cortex.Research`, `Cortex.Run`, or
...
Logos epistemological archetypes are not merely descriptive labels. Each archetype may correspond to
a concrete activation bundle: curated prompts, retrieval indexes, embedding spaces, tool
...
Current `Logos.<Archetype>.Capability` modules are staging stubs. The canonical target is
`Logos.Archetypes.<Archetype>.Activation`, populated by later PRs with real prompts, corpus
...
In Logos, `Logos` is the natural home for theorem-like reasoning, argument chains, decomposition,
...
In Logos, `Sophia` is the archetype most closely associated with high-level synthesis and executive
...
In Logos, `Techne` governs implementation-oriented thoughts and workflows: writing code, designing
...
In Logos, `Episteme` is the natural archetype for research thoughts, evidence gatherers, source
...
In Logos, `Kritikos` should be used to create adversarial branches, red-team analyses,
...
In Logos, `Themis` governs correctness checks, policy enforcement, compliance review, type
...
In Logos, `Poiesis` governs ideation, drafting, naming, design language, narrative construction,
...
- **No fixed archetype set.** Rejected because reusable Logos templates would keep accumulating
...
- **Downstream-specific roles only.** Rejected because evidence, reasoning, judgment,
  ...
  product may specialize them, but the generic taxonomy belongs in Logos.
...
This taxonomy gives Logos a stable, philosophically coherent language for modeling reasoning roles
...
Episteme" in the abstract; it runs with one or more Logos activations active. Runtime assembly can
...
The archetypes are intentionally broad enough to remain stable as Logos evolves on the Cortex
substrate, while precise enough to guide implementation, naming, module boundaries, thought
contracts, and workflow semantics.

### Obligations

- Expose archetype module stubs at exactly the canonical paths under `Logos.Archetypes`.
- Expose activation-bundle stubs at `Logos.Archetypes.<Archetype>.Activation`.
- Use `Logos.Patterns` for reusable LLM-shaped reasoning programs.
- Move existing direct `Logos.<Archetype>` and `Logos.<Archetype>.Capability` staging modules to the
  canonical `Logos.Archetypes.<Archetype>` and `Logos.Archetypes.<Archetype>.Activation` paths
