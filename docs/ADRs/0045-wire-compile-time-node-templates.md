...
commitment is the important part: a template call elaborates to an ordinary node declaration with an
ordinary typed boundary.

### Expansion phase

Template expansion is a static elaboration phase between parsing/import resolution and ordinary Wire
graph lowering. Expansion may use only source data already available at compile time:

- literal values;
- closed module-level pure data;
- configured executor values;
- imported or locally declared template definitions;
...
calls, Pulse state, or materialized artifacts. A template cannot inspect or modify the runtime
graph.
...
- a hole may bind only the syntactic class declared by the template parameter;
- port-label holes cannot accidentally capture value names;
- configured-executor holes cannot be called from CorePure;
- node-local `where` names inside the template body are scoped as if the expanded node had been
  handwritten at the call site.

Diagnostics must point to both the template call and the template body span that produced the
expanded form.

### Authority

Templates do not grant authority. They may reference only executor and contract names already
available through the normal registry, `use`, import, or configured-executor surfaces. Passing a
configured executor value into a template does not make that value a graph vertex; the expanded node
declaration remains the vertex.
...
This ADR does not add general graph templates, loops, generators, or higher-order graph functions.
Those are plausible future features, especially for parameterized sweeps such as the quantum eraser
phase table, but they add another decision: how a template expands multiple nodes and returns a
graph value with a stable boundary.
...
- Template expansion gives future tooling a clear place to attach origin metadata.

### Negative

- Wire gains another declaration kind and a static elaboration pass.
- Parser, formatter, tree-sitter, and reference docs must learn template declarations and calls.
...
- Prove or document the preservation claim: after template expansion, all existing Wire graph, port,
