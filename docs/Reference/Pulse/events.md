...
| -------------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `run.claimed`                    | info     | Written atomically by the claim transaction. Details carry `queue_seconds` (pending wait before claim) for first claims; a requeued run (resumed after waiting) records `requeued: true` and omits `queue_seconds` — its requeue time is unknown without a pending-since column (#259). Queue time and execution time are distinct clocks. |
...
| `run.signal_target_missing`      | error    | A runtime-owned run-terminal wait targeted a run id that does not exist; the wait is rejected instead of parking on an undeliverable signal.                                                                                                                                                                                               |
| `run.signal_wait_invalid`        | error    | Suspend settlement rejected an undeliverable or ambiguous wait, such as a run-terminal wait on the current run or a second node trying to reuse another node's pending signal name.                                                                                                                                                        |
