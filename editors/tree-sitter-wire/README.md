...
- bare executor authority values: `let analyst = @llm.analyst;`
...
- executor authority values: `@qualified.name`
- executor calls in node bodies: `@qualified.name`, `@qualified.name input`, or
  `@qualified.name { payload = input; cfg = { mode = "strict"; }; }`
...
- **Semantic checks are out of scope.** Tree-sitter accepts syntactically valid records, lists,
  executor authorities, executor calls, and graph expressions even when the compiler would reject
