...
compileWireTextWithEnv compileEnv sourceText = do
  wireFile <-
    mapLeft
      ...
      (parseWireFile "wire-v1" sourceText)
...
compileWireFragmentTextWithEnv compileEnv sourceText = do
  wireFile <-
    mapLeft
      (WireCore.WireParseError . renderParseError)
      (parseWireFile "wire-v1-fragment" sourceText)
...
compileLoweredWireFile compileEnv requireConnected wireFile lowered = do
  ...
  let metadataValue =
        fromMaybe
          (Aeson.object ["format" Aeson..= ("cortex.workflow.wire/v1" :: Text)])
          lowered.lwfMetadata
  pure
    CompiledCircuit
      ...
      Right
        LoweredWireFile
          { lwfFragment = resultFragment,
            ...
            lwfCircuitId = fromMaybe "wire_v1" (extractCircuitId maybeMetadata)
          }
      ...
      conditionNode =
        CircuitConditionNode
          { circuitConditionNodeRef = conditionRef,
            circuitConditionNodeCondition = CircuitConditionRef "__wire_v1_select__",
...
compatibilityWitness wireFile =
  CircuitCompatibilityWitness
    { circuitCompatibilityFamily = "cortex.workflow.wire/v1",
