...
import Cortex.Run.Engine qualified as Engine
...
runTextTask taskHost assistantPhase timeoutClass modelId groundingMode agentName maybeStep systemPrompt userInput = do
  runTextTaskWithMaxOutputTokens taskHost assistantPhase timeoutClass modelId groundingMode agentName maybeStep Nothing systemPrompt userInput
...
emitAgentStart :: (MonadIO m) => CortexTaskHost m -> CortexStageDescriptor -> Text -> Maybe Text -> m ()
...
emitAgentDone :: (MonadIO m) => CortexTaskHost m -> CortexStageDescriptor -> Text -> Maybe Text -> m ()
...
emitThinking :: (MonadIO m) => CortexTaskHost m -> CortexStageDescriptor -> Text -> Maybe Text -> m ()
...
emitSummary :: (MonadIO m) => CortexTaskHost m -> Maybe Text -> Maybe Text -> Maybe Int -> Text -> Maybe Text -> Maybe Aeson.Value -> m ()
