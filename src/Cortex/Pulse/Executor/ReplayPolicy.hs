                ...
                Left err ->
                  case policy of
                    ...
                    _ -> do
                      ...
                      now <- getCurrentTime
                      failRun pool runId now "replay_check_failed" ("Cannot verify replay safety for irreversible stage: " <> stageName) True
...
applyReplayPolicy pool runId policy stageName priorLogs = do
  ...
  case policy of
    ...
    ReplayPolicyBlockResume -> do
      ...
      now <- getCurrentTime
      failRun pool runId now "replay_blocked" ("Irreversible stage has prior execution history: " <> stageName) False
      pure (Just OutcomeFailed)
    ReplayPolicyRequireOperatorOverride -> do
      ...
      now <- getCurrentTime
      failRun pool runId now "replay_blocked_operator_override_required" ("Irreversible stage requires operator override: " <> stageName) False
      pure (Just OutcomeFailed)
