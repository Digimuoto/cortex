...
attemptStage env task stagePlan stageCall = do
  ...
  attemptLogIdResult <-
    DB.runTransaction env.sePool $
      Q.appendStageAttemptStarted stageCall.scLogId env.seRunId (fromIntegral stageCall.scAttempt) attemptStartedAt
  case attemptLogIdResult of
    Left err -> do
      emitObsEvent $ EvtStageAttemptLogFailed env.seRunId stageCall.scStageName (T.pack err)
      failRun env.sePool env.seRunId attemptStartedAt "internal_error" ("Stage attempt log failed: " <> T.pack err) False
  ...
  where
    ...
    dispatchStageResult attemptRef budgetCtx ctx reExecCount deferredRejections stageEnd = \case
      ...
      Right (StageSuspend signalName) -> do
        ...
        attemptResult <-
          DB.runTransaction env.sePool $
            Q.updateStageAttemptCompleted attemptRef.sarAttemptLogId StageCompleted (Just (Aeson.object ["suspended_on" Aeson..= unSignalName signalName])) stageEnd
      ...
      Left failure -> do
        ...
        case resolveStageFailure stageCall.scStageDef stageCall.scAttempt failure of
          ...
          FailRunAfterExhaustion failureSpec ->
            handleTerminalFailure env stageCall.scStageDef attemptRef stageCall.scStageName failure stageEnd failureSpec
    ...
    handleRejectedRewrite attemptRef budgetCtx ctx reExecCount deferredRejections stageEnd rejection = do
      ...
      emitObsEvent $ EvtRewriteRejected env.seRunId stageCall.scStageName (rewriteRejectionInfo rejectionCtx (Just rejectionCtx.rrcAttemptNumber))
        ...
        then do
          ...
          case guardCheck of
            StageTerminal outcome -> do
              ...
              _ <-
                DB.runTransaction env.sePool $ do
                  Q.updateStageAttemptCompleted attemptRef.sarAttemptLogId StageFailed (Just (Aeson.String "Aborted before rewrite re-execution")) abortedAt
                  Q.updateStageCompleted attemptRef.sarLogId StageFailed (Just (Aeson.String "Aborted before rewrite re-execution")) abortedAt
            ...
            _ -> do
              ...
              dispatchStageResult attemptRef reBudgetCtx reCtx (reExecCount + 1) deferredRejections' reEnd reResult
        else
  ...
  case exhaustionPolicyForRejection stagePlan rejection of
    RewriteExhaustionFail ->
      ...
      handleRewriteAdmissionFailure
        ...
        ( RunFailureSpec
            "rewrite_exhaustion"
            ("Max re-executions reached after " <> rejection.rrContext.rrcRejectionType <> ": " <> rejection.rrContext.rrcMessage)
            False
        )
        ( Just
            ( Aeson.object
    ...
    RewriteExhaustionDegrade -> do
      ...
      persistStageSuccess env attemptRef stageCall.scNodeId stageCall.scStageName stageEnd rejection.rrLastOutput
...
withPreAttemptGuards :: StageEnv -> Text -> IO (StageAttemptResult stageId) -> IO (StageAttemptResult stageId)
    ...
    Just () -> do
      ...
      case retryResult of
        StageTerminal OutcomeCancelled -> do
          ...
          result <-
            DB.runTransaction env.sePool $
              Q.updateStageCompleted attemptRef.sarLogId StageFailed (Just (Aeson.String "Cancelled before retry attempt")) cancelledAt
          case result of
            Left err ->
              recordRunEvent
                env
...
resolveStageFailure stageDef attempt failure =
  case stageDef.sdRetryPolicy of
    Nothing -> FailRunAfterExhaustion (RunFailureSpec (stageFailureType failure) (stageFailureMessage failure) True)
    Just retryPolicy ->
      let maxAttempts = max 1 retryPolicy.srpMaxAttempts
       ...
       in if retryable && attempt < maxAttempts
            then RetryStageAfter (retryDelayMicros retryPolicy.srpBackoff attempt)
              ...
              _ ->
                FailRunAfterExhaustion (RunFailureSpec (stageFailureType failure) (stageFailureMessage failure) retryable)
...
runStageAttempt task stageDef ctx =
  case effectiveStageTimeoutSeconds task stageDef of
    ...
    Just timeoutSeconds -> do
      timeoutResult <- Timeout.timeout (fromIntegral timeoutSeconds * 1_000_000) (tryAction (Just timeoutSeconds))
...
effectiveStageTimeoutSeconds :: PulseTaskDefinitionRow Result -> StageDefinition stageId -> Maybe Int32
