  ...
  where
    ...
    go frontier remaining !emitted acc results =
      case fextract frontier of
        ...
        Just (v, frontier') ->
          let
            -- Gather predecessor values for the step function.
           ...
           in
            go frontier'' remaining' (emitted + 1) (v : acc) results'
...
scc rel =
  let
    -- Phase 1: DFS on forward graph, producing vertices in decreasing
    ...
    revAdj = relPred rel
   in
    collectSCCs revAdj finishOrder
