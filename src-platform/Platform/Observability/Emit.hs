...
emitEvent (Just runtime) spec
  ...
  | otherwise = do
            ...
            object $
              catMaybes
                [ Just ("timestamp" .= now),
                  ...
                  ("payload_preview_visibility" .=) <$> fmap renderPreviewVisibility (eventPayloadPreviewVisibility spec),
      ...
      case droppedExtras of
        [] -> pure ()
        dropped ->
          hPutStrLn stderr ("observability: extra-field keys collide with built-in keys and were dropped: " <> show (fmap fst dropped))
