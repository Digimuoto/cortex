...
`RequiredNested` case: a nested required key inside a properties sub-schema.
...
namespace EmittedRequiredNested
...
def schema : SchemaNode :=
  SchemaNode.mk
      ...
      none
      ["filters"]
      [("filters", SchemaNode.mk ["object"] none ["region"] [] none none none none none none)]
      none
      none
      none
      none
...
def accepted0 : Json :=
  Json.obj [("filters", Json.obj [("region", Json.str "us")])]

/-- Payload the schema rejects. -/
def rejected0 : Json :=
  Json.obj [("filters", Json.obj [])]
