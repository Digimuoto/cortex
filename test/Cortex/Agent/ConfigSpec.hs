...
module Cortex.Agent.ConfigSpec (spec) where
...
spec = do
  describe "normalizeCortexAgentBudget" $ do
    it "drops non-positive values and clamps to the configured maxima" $ do
      normalizeCortexAgentBudget defaultBudgetLimits CortexAgentBudget {maxToolSteps = Just 0, maxAttempts = Just 99}
    ...
    it "returns Nothing when both limits normalize away" $ do
      normalizeCortexAgentBudget defaultBudgetLimits CortexAgentBudget {maxToolSteps = Just 0, maxAttempts = Just 0}
        `shouldBe` Nothing
  ...
  describe "validateCortexAgentBudget" $ do
    it "rejects values above the configured maxima" $ do
      validateCortexAgentBudget "agent.budget" defaultBudgetLimits CortexAgentBudget {maxToolSteps = Just 21, maxAttempts = Nothing}
