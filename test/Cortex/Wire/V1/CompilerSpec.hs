...
import Cortex.Circuit.IR (CircuitNodeRef (..), CircuitTaskNode (..))
import Cortex.Graph (successors)
import Cortex.Wire.V1 (compileWireFragmentText, compileWireText)
...
spec :: Spec
spec = describe "Cortex.Wire.V1.Compiler" $ do
  it "compiles a minimal strict-v1 chain through the compiled circuit backend" $ do
  ...
  it "compiles a wrapper-bearing v1 file and lifts runtime-wrapper metadata" $ do
